...
- **Target frameworks:** `Stt.Abstractions`, `Stt.Core`, both test projects = `net8.0`. `Stt.Audio.Windows` = `net8.0-windows`. `Stt.App` = `net8.0-windows10.0.19041.0`. (Spec §4.)
  - **Update (post-implementation, 2026-06-28):** the solution was migrated to **.NET 10** at the user's request — all projects now target `net10.0` / `net10.0-windows` / `net10.0-windows10.0.19041.0`, and `Stt.App` moved to **Windows App SDK 2.2** (from 1.6). The CommunityToolkit `SettingsControls` package (no Windows App SDK 2.x build exists) was dropped in favour of a native `Stt.App.Controls.SettingCard` + the built-in `Expander`; only `CommunityToolkit.Mvvm` (UI-framework-agnostic) remains. `System.Threading.Channels` is now in-box on net10 (explicit package reference removed). **ONNX Runtime** was bumped `Microsoft.ML.OnnxRuntime` 1.20.1 → **1.27.0** (managed API unchanged — verified by an adversarial assembly diff; the only removals in that range were the unused TVM EP methods) and the optional plugin `Microsoft.ML.OnnxRuntimeGenAI` 0.8 → **0.14.1** (one compile-breaking change: `GeneratorParams.SetInputs` moved to `Generator.SetInputs`, fixed). Because WinApp SDK 2.2 transitively bundles WinML's own ORT 1.24.6 `onnxruntime.dll` at the same RID path, `Stt.App` excludes the WinML package's runtime/native assets so ORT 1.27.0 is the single deterministic owner of `onnxruntime.dll` (see docs/native/execution-providers.md). The solution was also converted from the classic `Stt.sln` to the XML `Stt.slnx` format (`dotnet sln migrate`). All 117 tests still pass (4 skipped).
...
- [ ] **Step 1:** Run full `dotnet build Stt.slnx` (App excluded on non-Windows CI via solution filter or `-p:`) — document which projects build where.
