...
/// <summary>
/// Builds the initial (zero) encoder state tensors for a streaming Zipformer2 transducer from its
/// metadata-derived geometry (spec §8.2). Per encoder layer there are 6 caches (key / nonlin_attn
/// / val1 / val2 / conv1 / conv2); plus two global states (embed_states + processed_lens), giving
/// <c>m*6 + 2</c> tensors. Shapes follow sherpa-onnx; nothing is hardcoded beyond the Zipformer2
/// structural constants. Numerical correctness is confirmed by aligning against sherpa-onnx output
/// before the streaming decoder is trusted (Task 1.3).
/// </summary>
